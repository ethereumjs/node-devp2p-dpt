...
function parseIp (ip) {
  assert(ip.length === 4 || ip.length === 16)

  if (ip.length === 4) {
    var ret = new ipaddr.IPv4(ip).toString()
    return ret === '0.0.0.0' ? '127.0.0.1' : ret
  } else {
    // FIXME: hopefully the PR will be merged in
    var tmp = []
    for (var i = 0; i < 16; i += 2) {
      tmp.push((ip[0] << 8) | ip[1])
    }
    return new ipaddr.IPv6(tmp).toString()
  }
}

function createIp (str) {
  return new Buffer(ipaddr.parse(str).toByteArray())
}
...
var DPT = module.exports = function (opts) {
  ...
  this.address = opts.address || this.publicAddress
  ...
  this.id = opts.id || ecdsa.publicKeyCreate(this.secKey, false).slice(1)
...
DPT.prototype.parsePacket = function (raw, rinfo) {
  ...
  if (h.toString('hex') !== hash.toString('hex')) {
    ...
    rinfo.id = ecdsa.recover(signHash, signature, recovery, false).slice(1)
  ...
  function recFunc (pong, rpeer) {
    if (peer.udpPort === rpeer.port && peer.address === rpeer.address) {
      ...
      if (pong.hash.toString('hex') !== hash.toString('hex')) {
        ...
        rpeer.tcpPort = peer.tcpPort
        self.kBucket.add(rpeer)
        this.emit('newPeer', rpeer)
        cb(undefined, rpeer)
      }
    }
  }
...
DPT.prototype._pong = function (hash, peer, cb) {
  const data = [
    [createIp(this.address), ethUtil.intToBuffer(peer.udpPort), ethUtil.intToBuffer(peer.tcpPort)],
    hash
  ]
...
/**
 * Returns a filtered list of available nodes or all if no filter is supplied.
 * @method filterPeers
 * @param {Function} filter a filter function, which receiver a `peer` structure as parameter
 */
DPT.prototype.filterPeers = function (filter) {
  var peers = this.kBucket.toArray()
  if (filter) {
    return peers.filter(filter)
  } else {
    return peers
  }
}
...
/**
 * Returns a single peer object if found, otherwise null.
 * @method getPeer
 * @param {Buffer} id the node id of the peer
 */
DPT.prototype.getPeer = function (id) {
  return this.kBucket.get(id)
}
