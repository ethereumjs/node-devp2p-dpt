...
function parseIp (ip) {
  ...
  if (ip.length == 4) {
    ...
    return new ipaddr.IPv6(ip).toString()
  }
}

function createIp (str) {
  if (ipaddr.IPv4.isValid(str)) {
    return new Buffer(ipaddr.IPv4.parse(str).octets)
  } else {
    return new Buffer(ipaddr.IPv6.parse(str).parts)
  }
}
...
DPT.prototype.filterPeers = function (filter) {
  var peers = self.kBucket.toArray()
  if (filter) {
